...
def set_project(name, layout, cost, sched, fa_settings=None, display=None):
    ...
    # A tag filter is built from the tags of the layout that was on screen. Carried
    # over to a new layout it matches nothing, and because the filter controls are
    # only shown when the layout HAS tags, there is no way to clear it — the map
    # just goes blank. Clear it with the layout it belonged to.
    st.session_state.tag_include, st.session_state.tag_exclude = [], []
...
def _unique_case_name(base: str) -> str:
    names = {c_["name"] for c_ in st.session_state.get("cases", [])}
...
def _stash_current_concept():
    """Keep what is on screen as a concept, so loading another does not lose it."""
    S = st.session_state
    S.cases = S.get("cases") or []
    cur = S.get("active_case") or WORKING_LAYOUT
    snap = lambda nm: tb_cases.snapshot(nm, S.layout, S.cost_settings, S.sched_settings,  # noqa: E731
                                        S.fa_settings, "", S.display)
    if cur != WORKING_LAYOUT and any(c_["name"] == cur for c_ in S.cases):
        S.cases = [c_ for c_ in S.cases if c_["name"] != cur]
        S.cases.append(snap(cur))
        return cur
    if S.layout.nodes:
        name = _unique_case_name(S.project_name or "Working layout")
        S.cases.append(snap(name))
        return name
    return None


def _load_as_concept(name, layout, cost, sched, fas, display=None) -> str:
    """Load a template or project alongside the current one instead of replacing it."""
    S = st.session_state
    kept = _stash_current_concept()
    new = _unique_case_name(name)
    set_project(new, layout, cost, sched, fas, display)
    S.cases.append(tb_cases.snapshot(new, layout, cost, sched, fas, "", S.display))
    S.active_case = new
    S.case_rows = None
    S.ghost_cases = [g for g in (S.get("ghost_cases") or []) if g != new]
    if kept and kept not in S.ghost_cases:
        S.ghost_cases.append(kept)     # draw what you were on next to what you just loaded
    return new
...
with st.sidebar:
    ...
    if S.get("loaded_note"):
        st.success(S.loaded_note)
        S.loaded_note = ""

    # ── Concepts: several layouts kept side by side ──
    st.subheader("Concepts")
    S.cases = S.get("cases") or []
    names_c = [c_["name"] for c_ in S.cases]
    current = S.get("active_case") or WORKING_LAYOUT
    options_c = [WORKING_LAYOUT] + names_c
    if current not in options_c:
        current = WORKING_LAYOUT
    pick_active = st.selectbox(
        "Concept being edited", options_c, index=options_c.index(current),
        key=f"active_case_{REV}",
        help="Everything else follows this one — cost, schedule, flow assurance, viability, report")
    if pick_active != current:
        if S.get("autosave_case", True) and current in names_c:
            ...
            S.cases.append(tb_cases.snapshot(current, LAY, S.cost_settings, S.sched_settings,
                                             S.fa_settings, "", S.display))
        if pick_active != WORKING_LAYOUT:
            set_project(*tb_cases.restore(next(c_ for c_ in S.cases if c_["name"] == pick_active)))
        S.active_case = pick_active
        S.case_rows = None
        S.ghost_cases = [g for g in (S.get("ghost_cases") or []) if g != pick_active]
        st.rerun()

    if names_c:
        S.ghost_cases = st.multiselect(
            "Also draw on the map", [n_ for n_ in names_c if n_ != pick_active],
            default=[g for g in (S.get("ghost_cases") or []) if g in names_c and g != pick_active],
            key=f"ghost_cases_{REV}",
            help="Drawn dashed in each concept's colour, over the routes of the one you are editing")
        if S.ghost_cases:
            st.markdown("".join(
                f'<div style="display:flex;align-items:center;gap:7px;margin:1px 0">'
                f'<span style="width:20px;border-top:3px dashed {tb_cases.color_for(S.cases, nm)}"></span>'
                f'<span style="font-size:12px">{nm}</span></div>' for nm in S.ghost_cases),
                unsafe_allow_html=True)

    cbtn = st.columns(2)
    if cbtn[0].button("Save as concept", help="Snapshot the layout on screen under the project name"):
        nm = _unique_case_name(S.project_name or "Concept")
        S.cases.append(tb_cases.snapshot(nm, LAY, S.cost_settings, S.sched_settings, S.fa_settings,
                                         "", S.display))
        S.active_case = nm
        S.case_rows = None
        st.rerun()
    if names_c and cbtn[1].button("Delete concept", disabled=pick_active == WORKING_LAYOUT):
        S.cases = [c_ for c_ in S.cases if c_["name"] != pick_active]
        S.ghost_cases = [g for g in (S.get("ghost_cases") or []) if g != pick_active]
        S.active_case = WORKING_LAYOUT
        S.case_rows = None
        st.rerun()
    S.autosave_case = st.checkbox("Save edits when switching", S.get("autosave_case", True))
    if names_c:
        st.caption(f"{len(names_c)} concept(s) saved · compare them on cost and schedule in the Cases tab")
        ...
        with st.spinner("Sampling the seabed along each line…"):
            try:
                ...
                if ok:
                    ...
                    st.warning("No profiles returned for those routes — press *Test EMODnet "
                               "connection* below to see whether the service answered at all.")
    ...
    if not has_api("tb_bathymetry", "diagnose"):
        ...
        with st.spinner("Probing rest.emodnet-bathymetry.eu…"):
            lat0, lon0 = (60.5, 2.5)
            if LAY.nodes:
                n0 = next(iter(LAY.nodes.values()))
                lat0, lon0 = tb_map.to_display(LAY, n0.lat, n0.lon)
            rep = tb_bathymetry.diagnose(_session(), lat0, lon0)
        msg = tb_bathymetry.diagnosis_message(rep)
...
with tab_layout:
    ...
    if S.get("ghost_cases"):
        active = S.get("active_case") or WORKING_LAYOUT
        st.markdown(
            '<div style="margin:0 0 6px">'
            '<span style="display:inline-flex;align-items:center;gap:6px;margin-right:16px">'
            '<span style="width:22px;border-top:3px solid #00243D"></span>'
            f'<span style="font-size:13px"><b>{active}</b> — editing</span></span>'
            + "".join(
    ...
    if (tag_inc or tag_exc) and not LAY.all_tags():
        tag_inc, tag_exc = [], []          # nothing in this layout is tagged — the filter cannot apply
        S.tag_include, S.tag_exclude = [], []
    visible = set(LAY.by_tags(tag_inc, tag_exc)) if (tag_inc or tag_exc) else None
    if visible is not None:
        total = len(LAY.nodes) + len(LAY.edges)
        if not visible:
            fc_ = st.columns([3, 1])
            fc_[0].warning("The tag filter matches nothing, so the map is empty.")
            if fc_[1].button("Clear tag filter"):
                S.tag_include, S.tag_exclude = [], []
                st.rerun()
        else:
